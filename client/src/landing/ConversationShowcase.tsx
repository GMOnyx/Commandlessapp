...
type ScriptLine = { role: Role; text: string };

const Avatar = ({ role }: { role: Role }) => (
  <div className={`mt-1 flex h-8 w-8 items-center justify-center rounded-full border ${role === "user" ? "bg-primary text-white border-primary" : "bg-muted text-foreground"}`}>
...
const Row = ({ role, children }: { role: Role; children: React.ReactNode }) => (
  <div className={`flex ${role === "user" ? "justify-end" : "justify-start"} items-start gap-3`}>
    ...
    <div className={`max-w-[80%] rounded-2xl px-4 py-3 text-sm md:text-base shadow-sm border animate-in fade-in slide-in-from-bottom-2 ${role === "user" ? "bg-primary text-white border-primary" : "bg-background text-foreground"}`}>
      {children}
    </div>
...
export default function ConversationShowcase() {
  ...
  useEffect(() => {
    ...
    if (visibleCount >= script.length) { setShowTyping(false); return; }
    const current = script[visibleCount];
    if (current.role === "assistant") {
      setShowTyping(true);
      const typingTimer = setTimeout(() => { setShowTyping(false); setVisibleCount((c) => c + 1); }, 900);
