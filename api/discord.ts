...
async function processMessageWithAI(
  ...
  try {
    ...
    if (genAI) {
      // Build conversation context from MessageContextManager like server/simple-index.js
    let conversationContext = '';
    if (message.reference const result = await processWithAI(cleanMessage, commandMappings, message, userId, '');const result = await processWithAI(cleanMessage, commandMappings, message, userId, ''); message.reference.messageId) {
    ...
    const recentMessages = messageContextManager.getRecentMessages(message.channel_id, 3);
    if (recentMessages.length > 0) {
      const contextMessages = recentMessages
        .filter(msg => msg.messageId !== message.id)
    ...
    const result = await processWithAI(cleanMessage, commandMappings, message, userId, conversationContext);
      return {
...
// MISSING CRITICAL FUNCTIONS (Phase 2 completion)
...
// Find best command match using advanced algorithms (from server/simple-index.js)
async function findBestCommandMatch(userInput: string, availableCommands: any[], mentionedUserIds: string[] = []): Promise<any> {
...
// Enhanced MessageContextManager for conversation tracking (migrated from server/simple-index.js)
class MessageContextManager {
  private contexts: Map<string, any[]> = new Map(); // channelId -> messages
  ...
  getRecentMessages(channelId: string, limit: number = 5): any[] {
    const messages = this.contexts.get(channelId) || [];
    return messages.slice(-limit);
  }

  getMessageById(channelId: string, messageId: string): any {
    const messages = this.contexts.get(channelId) || [];
