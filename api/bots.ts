...
async function discoverAndSyncCommands(botToken: string, botId: string, userId: string, forceRefresh: boolean = false) {
  try {
    ...
    // Old method: do not generate per-mapping aliases/examples here. Keep only the mapping upsert.
...
// Reverted: no per-mapping AI metadata generation here; rely on the original pattern-based flow.
