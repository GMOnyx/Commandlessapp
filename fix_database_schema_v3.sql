...
-- This version finds and drops ALL policies dynamically

-- Step 1: Drop ALL policies on ALL tables (comprehensive list)
...
DROP POLICY IF EXISTS "cmd_map_policy" ON command_mappings;
...
-- Drop any default policies
DROP POLICY IF EXISTS "Enable read access for all users" ON users;
DROP POLICY IF EXISTS "Enable read access for all users" ON bots;
DROP POLICY IF EXISTS "Enable read access for all users" ON command_mappings;
